...
export class LoginService {
  ...
  constructor(
    ...
    this.loggedUser = new BehaviorSubject<User>(JSON.parse(localStorage.getItem(USER_API_STORAGE_KEY)));
  }
  ...
  authenticate(user: User): Observable<any> {
    ...
    return this.http.get<User>(this.baseUrl + `user/login`, {
      params: userParams,
    }).pipe(
      tap((response) => {
        userParams.session  = response.message.substr(response.message.indexOf(':') + 1);
        delete userParams.password;
        localStorage.setItem(USER_API_STORAGE_KEY, JSON.stringify(userParams));
